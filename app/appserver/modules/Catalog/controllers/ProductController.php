...
namespace fbbcbase\app\appserver\modules\Catalog\controllers;
...
class ProductController extends \fecshop\app\appserver\modules\Catalog\controllers\ProductController
{
    ...
    public function actionIndex()
    {
        ...
        if(is_array($gallerys) && !empty($gallerys)){
            foreach($gallerys as $gallery){
                $i++;
                $image = $gallery['image'];
                $resizeImage = Yii::$service->product->image->getResize($image,$middle_img_width,false);
                if ($i == 1) {
                    $main_img = $resizeImage;
                }
                $thumbnail_img[] = $resizeImage;
            }
        }
        ...
        $data = [
            ...
            'servicesPhone' => $this->getServicesPhone(),
        ];
        $responseData = Yii::$service->helper->appserver->getResponseData($code, $data,$message);
    ...
    // 得到服务电话
    public function getServicesPhone(){
        $phone = '';
        // 登陆用户直接取供应商对应的电话
        if (!Yii::$app->user->isGuest) {
