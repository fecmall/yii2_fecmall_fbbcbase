 ...
 * @since 1.0
 */
class EditaccountController extends \fecshop\app\appserver\modules\Customer\controllers\EditaccountController
{
    public $enableCsrfValidation = false ;
    public $show_last_name = false;
    /**
     * 登录用户的部分
     */
    ...
    public function actionUpdate(){
        ...
        if ($this->show_last_name && $lastname) {
    ...
    public function validateParams($identity ,$firstname,$lastname,$current_password,$new_password,$confirm_new_password){
        ...
        if ($current_password && !$new_password) {
            $errorArr[] = Yii::$service->page->translate->__('new password can not empty');
        } elseif ($current_password && ($new_password != $confirm_new_password)) {
            $errorArr[] = Yii::$service->page->translate->__('Password and confirmation password must be consistent');
            
        } elseif ($current_password && (strlen($new_password) < $minPassLength || strlen($new_password) > $maxPassLength)) {
            $errorArr[] = Yii::$service->page->translate->__('password must >= {minPassLength} and <= {maxPassLength}', ['minPassLength' => $minPassLength, 'maxPassLength'=> $maxPassLength]);
           
        } elseif (strlen($firstname) < $minNameLength || strlen($firstname) > $maxNameLength) {
            $errorArr[] = Yii::$service->page->translate->__('firstname must >= {minPassLength} and <= {maxPassLength}', ['minPassLength' => $minPassLength, 'maxPassLength'=> $maxPassLength]);
            
        } elseif ($this->show_last_name && (strlen($lastname) < $minNameLength || strlen($lastname) > $maxNameLength)) {
            $errorArr[] = Yii::$service->page->translate->__('lastname must >={minPassLength} and <= {maxPassLength}', ['minPassLength' => $minPassLength, 'maxPassLength'=> $maxPassLength]);
        }
        
        if($current_password && !$identity->validatePassword($current_password)){
            $errorArr[] = Yii::$service->page->translate->__('current password is not correct');
        }
        if(!empty($errorArr)){
