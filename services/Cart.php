...
class Cart extends \fecshop\services\Cart
{
    ...
    protected function actionAddProductToCart($item)
    {
        ...
        if (isset($item['custom_option_sku']) && !empty($item['custom_option_sku']) && is_array($item['custom_option_sku'])) {
            ...
            if (is_array($product_custom_option) && !empty($product_custom_option)) {
                Yii::$service->helper->errors->add('item custom_option_sku can not empty');
