...
namespace fbbcbase\services\helper;
...
class Appserver extends \fecshop\services\helper\Appserver
{
    ...
    public $status_role_refuse                          = 1000014;   // 没有权限
    ...
    public $order_payment_method_update_fail             = 1510021;
     ...
     * customer order
     */
    public $customer_order_cancel_request_fail      = 3000001; // 用户发起取消订单失败。
    public $customer_order_return_request_fail      = 3000002; // 用户退货申请失败。
    public $customer_order_return_dispatch_fail      = 3000003; // 用户退货发货失败。
    public $customer_order_cancel_back_fail      = 3000004; // 用户撤销取消订单失败。
    public $customer_order_return_cancel_fail      = 3000005; // 用户退货发起后，撤销（取消）退货失败。
    public $customer_order_return_is_empty = 3000006; // 用户订单退款信息为空
    public $customer_default_address_is_null = 3000007; // 用户的默认货运地址为空
    public $customer_set_default_address_fail = 3000008; // 设置customer的默认收货地址
    public $customer_remove_address_fail = 3000009; // remove customer收货地址 失败
    public $customer_order_delay_receive_fail = 3000010;  // customer 延迟订单收货时间失败
    public $account_login_bdmin_user_id_is_empty = 3000011; // customer 检测bdmin_user_id是否为空，如果为空则不能登陆
    ...
    public function getMessageArr()
    {
        $arr = [
            ...
            $this->status_role_refuse => [
                'message' => 'system error, role refuse',
            ],
            ...
            $this->order_payment_method_update_fail => [
                'message' => 'order payment method update fail',
            ],
            
            $this->customer_order_cancel_request_fail => [
                'message' => 'customer  order cancel request fail',
            ],
            
            $this->customer_order_return_request_fail => [
                'message' => 'customer  order return request fail',
            ],
            $this->customer_order_return_dispatch_fail => [
                'message' => 'customer  order return dispatch fail',
            ],
            
            $this->customer_order_cancel_back_fail => [
                'message' => 'customer  order cancel back fail',
            ],
            $this->customer_order_return_cancel_fail => [
                'message' => 'customer  order return cancel fail',
            ],
            $this->customer_order_return_is_empty => [
                'message' => 'customer  order return is empty',
            ],
            $this->customer_default_address_is_null => [
                'message' => 'customer  default address is null',
            ],
            $this->customer_set_default_address_fail => [
                'message' => 'set customer default address fail',
            ],
            
            $this->customer_remove_address_fail => [
                'message' => 'remove customer default address fail',
            ],
            
            $this->customer_order_delay_receive_fail => [
                'message' => 'customer order delay recevie time fail',
            ],
            $this->account_login_bdmin_user_id_is_empty => [
                'message' => 'account login bdmin user id is empty',
            ],
